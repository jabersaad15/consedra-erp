...
export class Org-structureService {
  ...
  async findAll(tenantId: string, params: any) {
    ...
    if (params.search) {
      where.OR = [{ name: { contains: params.search, mode: 'insensitive' } }, { code: { contains: params.search, mode: 'insensitive' } }];
    }
    const [data, total] = await Promise.all([
      this.prisma.region.findMany({ where, skip, take: limit, orderBy }),
  ...
  async create(tenantId: string, data: any) {
    return this.prisma.region.create({ data: { ...data, tenantId } });
  }
